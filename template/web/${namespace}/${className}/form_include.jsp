...
<!--if have trigger -->
<#if table.hasTrigger>
<script type="text/javascript" src="${'$'}{ctx}/js/pages/${className}.js"></script>
</#if>
<!-- End if have trigger -->
<script type="text/javascript" src="${'$'}{ctx}/js/ckeditor/ckeditor.js"></script>
<script language="javascript" type="text/javascript" src="${'$'}{ctx}/js/mytimectrl.js"></script>
<script language="javascript" type="text/javascript" src="${'$'}{ctx}/js/My97DatePicker/WdatePicker.js"></script>
<link href="${'$'}{ctx}/js/My97DatePicker/skin/default/datepicker.css" rel="stylesheet" type="text/css"/>
		...
		<#else>		   
		    ...
		    <#elseif column.editable!="false">
		    	...
		    	<#else>
		   		...
		   		if(!CheckDefaultString(document.getElementById('${column.columnNameLower}'),'${column.remarks}','${column.size}','${column.mandatory}','${column.validate}'))
					return false;
					<#elseif column.javaType="java.math.BigDecimal" ||  column.javaType="BigDecimal">
						if(!CheckDefaultDecimal(document.getElementById('${column.columnNameLower}'),'${column.remarks}','${column.size}','${column.mandatory}','${column.validate}'))
					return false;		
		   			<#elseif column.sqlTypeName="int">
		   		if(!CheckDefaultInt(document.getElementById('${column.columnNameLower}'),'${column.remarks}','${column.size}','${column.mandatory}','${column.validate}'))
					return false;
		   			</#if>
		   		</#if>
			</#if>
...
<#list table.columns as column>		
	...
	function viewimage(id){
  		if(document.getElementById(id).value!=""){
  			var tmpurl = '<s:property value="%{resourcebase}"/>'+'/'+  document.getElementById(id).value;	
  			window.open(tmpurl);
  		}
  	}
	...
	function dialog_filecallback(result,idname,resultdata){
 		if(resultdata!=null){
 			//$('${'#ahref__'}'+idname).attr('href',resultdata.fullurl);
   	...
   	function dialog_imagecallback(result,idname,resultdata){
 		if(resultdata!=null){
 			...
 			$('#img__'+idname).attr('src',resultdata.fullurl); 		
 		} 	
  	}
 	...
 	function selectFile(type,s,idname,folder){
 		if(type=='ftp')
 			JqueryDialog.Open(dialog_ftpfilecallback,idname,'选择', '${'$'}{ctx}/fileBrowser.do', 720, 400);
 		else if(type=='image')
 		 	JqueryDialog.Open(dialog_imagecallback,idname,'选择', '${'$'}{ctx}/upload.jsp?currentpath='+folder, 720, 400);
 		else JqueryDialog.Open(dialog_filecallback,idname,'选择', '${'$'}{ctx}/upload.jsp?currentpath='+folder, 720, 400);
...
<#if !column.htmlHidden>
	...
	<#else>
			...
			<#elseif column.displayType="file">
				...
				<tr>
				<td style="width:20px;border:0px;">
					<img src="${'$'}{ctx}/images/edit/icon-upload.gif" onclick="selectFile('http','${table.className}','${column.columnNameLower}','${column.uploadfolder}')"/>
				</td>
				<td style="width:20px;border:0px;">
					<img src="${'$'}{ctx}/images/edit/icon-view.gif" border="0" onclick="viewfile('${column.columnNameLower}')"/>
				</td>
				<td style="border:0px;">									   								
			...
			<#elseif column.displayType="imagefile">
				...
				<tr>
				<td style="width:20px;border:0px;">
					<img src="${'$'}{ctx}/images/edit/getbyid.gif" onclick="selectFile('image','${table.className}','${column.columnNameLower}','${column.uploadfolder}')"/>
				</td>
				<td style="border:0px;">					
					<img id="img__${column.columnNameLower}" src="<s:property value="resourcebase"/>/<s:property value="%{model.${column.columnNameLower}}"/>" width="30" height="30" border="0" onclick="viewimage('${column.columnNameLower}')"/>
			...
			<#elseif column.displayType="ftpfile">
				...
				<tr>
				<td style="width:20px;border:0px;">
					<img src="${'$'}{ctx}/images/edit/getbyid.gif" onclick="selectFile('ftp','${table.className}','${column.columnNameLower}','${column.uploadfolder}')"/>
				</td>
				<td style="border:0px;">
					<s:textfield label="%{@${basepackage}.model.${className}@ALIAS_${column.constantName}}" id="${column.columnNameLower}" name="${column.columnNameLower}" value="%{model.${column.columnNameLower}}" cssClass="inputClass"  theme="simple"/>
				</td>			
			...
			<!--headerKey="0"   headerValue="请选择" -->
				<#if column.dataSrcType="1">
								<#if column.editable="false">
					<s:select disabled="true" label="%{@${basepackage}.model.${className}@ALIAS_${column.constantName}}" id="${column.columnNameLower}" name="${column.columnNameLower}" value="%{model.${column.columnNameLower}}" list="${column.enumComboxList}" cssClass="comboxcss" required="${(!column.nullable)?string}" theme="simple"/>								
								<#else>
				...
				<#else>
					<#if column.editable="false">
					<s:select disabled="true" label="%{@${basepackage}.model.${className}@ALIAS_${column.constantName}}" id="${column.columnNameLower}" name="${column.columnNameLower}" value="%{model.${column.columnNameLower}}" list="%{${column.fkalias?uncap_first}Options}"  listKey="value" listValue="name"  cssClass="comboxcss" required="${(!column.nullable)?string}" theme="simple"/>					
					<#else>
					<s:select label="%{@${basepackage}.model.${className}@ALIAS_${column.constantName}}" id="${column.columnNameLower}" name="${column.columnNameLower}" value="%{model.${column.columnNameLower}}" list="%{${column.fkalias?uncap_first}Options}"  listKey="value" listValue="name"  cssClass="comboxcss" required="${(!column.nullable)?string}" theme="simple"/>
					</#if>
				</#if>
			<#elseif column.displayType="unixtime">
				<#if column.editable="false">
					<s:hidden id="${column.columnNameLower}" name="${column.columnNameLower}" value="%{model.${column.columnNameLower}}"/>
					<s:textfield label="%{@${basepackage}.model.${className}@ALIAS_${column.constantName}}" id="${column.columnNameLower}AsDateTime" name="${column.columnNameLower}AsDateTime" value="%{model.${column.columnNameLower}AsDateTime}" cssClass="inputClass" required="${(!column.nullable)?string}"  theme="simple" readonly="true"/>
				<#elseif column.generate=="">
					...
					<s:hidden id="${column.columnNameLower}" name="${column.columnNameLower}" value="%{model.${column.columnNameLower}}"/>				
	  				...
	  				style="width:80px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd',onpicked:function(dp){ trigger_onChange('${column.columnNameLower}','${column.trigger}'); return false;}})" value="<s:property value="%{model.${column.columnNameLower}AsDateTime}"/>"/>	  				
	  					<#else>
	  						<input type="text" class="Wdate" id="${column.columnNameLower}AsDateTime" name="${column.columnNameLower}AsDateTime" 
	  				style="width:80px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd'})" value="<s:property value="%{model.${column.columnNameLower}AsDateTime}"/>"/>	  				
	  					</#if>		
	  				<#else>
	  					<#if column.trigger!="">	
	  					<input type="text" class="Wdate" id="${column.columnNameLower}AsDateTime" name="${column.columnNameLower}AsDateTime" 
	  						style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss',onpicked:function(dp){ trigger_onChange('${column.columnNameLower}','${column.trigger}'); return false;}})" value="<s:property value="%{model.${column.columnNameLower}AsDateTime}"/>"/>
	  					<#else>
	  						<input type="text" class="Wdate" id="${column.columnNameLower}AsDateTime" name="${column.columnNameLower}AsDateTime" 
	  						style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="<s:property value="%{model.${column.columnNameLower}AsDateTime}"/>"/>
			...
			<#elseif column.displayType="datetime">
				...
				<#elseif column.isDateTimeColumn>			
					<input type="text" class="Wdate" id="${column.columnNameLower}AsString" name="${column.columnNameLower}AsString" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss',onpicked:function(dp){ trigger_onChange('${column.columnNameLower}','${column.trigger}'); return false;}})" value="${'$'}{model.${column.columnNameLower}AsString}"/>					
				<#else>
					<#if column.dataformat=="yyyy-MM-dd hh:mm:ss">
						<#if column.trigger!="">	
							<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss',onpicked:function(dp){ trigger_onChange('${column.columnNameLower}','${column.trigger}'); return false;}})" value="${'$'}{model.${column.columnNameLower}}"/>
						<#else>
						<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="${'$'}{model.${column.columnNameLower}}"/>						
						</#if>
					<#elseif column.dataformat=="yyyy-MM-dd">
						<#if column.trigger!="">	
							<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd',onpicked:function(dp){ trigger_onChange('${column.columnNameLower}','${column.trigger}'); return false;}})" value="${'$'}{model.${column.columnNameLower}}"/>
						<#else>
							<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyy-MM-dd'})" value="${'$'}{model.${column.columnNameLower}}"/>
						</#if>					
					<#elseif column.dataformat=="yyyyMMdd">
						<#if column.trigger!="">	
							<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyyMMdd',onpicked:function(dp){ trigger_onChange('${column.columnNameLower}','${column.trigger}'); return false;}})" value="${'$'}{model.${column.columnNameLower}}"/>
						<#else>
							<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyyMMdd'})" value="${'$'}{model.${column.columnNameLower}}"/>
						</#if>										
					<#else>
						<#if column.trigger!="">	
							<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyyMMddHHmmss',onpicked:function(dp){ trigger_onChange('${column.columnNameLower}','${column.trigger}'); return false;}})" value="${'$'}{model.${column.columnNameLower}}"/>
						<#else>
							<input type="text" class="Wdate" id="${column.columnNameLower}" name="${column.columnNameLower}" style="width:140px" onFocus="WdatePicker({isShowWeek:true,dateFmt:'yyyyMMddHHmmss'})" value="${'$'}{model.${column.columnNameLower}}"/>
						</#if>
					</#if>
				</#if>
				</#if>
			...
			<#else>						
				<#if column.editable="false">
	  			...
	  			<#else>
	  				<#if column.trigger!="">	  				
	  					<s:textfield label="%{@${basepackage}.model.${className}@ALIAS_${column.constantName}}" id="${column.columnNameLower}" name="${column.columnNameLower}" value="%{model.${column.columnNameLower}}" cssClass="inputClass" required="${(!column.nullable)?string}"  theme="simple" onChange="trigger_onChange('${column.columnNameLower}','${column.trigger}')"/>
	  				<#else>
