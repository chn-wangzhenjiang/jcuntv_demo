...
弹出窗口
**/
function openDialog(url,name,w,h,l,t){
	window.showModelessDialog(url,name,'dialogWidth:'+w+'px;dialogHeight:'+h+'px;dialogLeft:'+l+'px;dialogTop:'+t+'px;center:yes;help:no;resizable:yes;status:no;edge:sunken');
	//window.open(url,name,'height='+h+',width='+w+',top='+t+',left='+l+',location=no,toolbar=no,menubar=no,scrollbars=no,scroll=yes,alwaysRaised=yes,directories=no,z-look=yes,resizable=no,status=yes');
}