			...
			<table>
				<tr>	
						...
						<td class="tdLabel"><%=SysUser.ALIAS_EMAIL%></td>		
							<td>
						<input value="${query.email}" id="email" name="email" maxlength="255"  class="validate-email "/>
					</td>
				</tr>	
				<tr>	
						...
						<td class="tdLabel"><%=SysUser.ALIAS_PHONE_NUMBER%></td>		
							<td>
						<input value="${query.phoneNumber}" id="phoneNumber" name="phoneNumber" maxlength="255"  class=""/>
					</td>
					
						<td class="tdLabel"><%=SysUser.ALIAS_PASSWORD_HINT%></td>		
							<td>
						<input value="${query.passwordHint}" id="passwordHint" name="passwordHint" maxlength="255"  class=""/>
					</td>
					
						<td class="tdLabel" colspan="2">
							<table align="left">
							<s:select label="%{@com.company.project.model.SysUser@ALIAS_ACCOUNT_TYPE}" key="accountType" value="query.accountType" headerKey=""   headerValue="请选择" list="#{'1':'系统点数','2':'人民币','3':'美元'}" cssClass="comboxcss" required="false"/>
							</table>
						</td> 
					</td>
						...
						<td class="tdLabel"><%=SysUser.ALIAS_CREATE_DATE%></td>		
							<td>
						<input value="<fmt:formatDate value='${query.createDateBegin}' pattern='<%=SysUser.FORMAT_CREATE_DATE%>'/>" onclick="WdatePicker({dateFmt:'<%=SysUser.FORMAT_CREATE_DATE%>'})" id="createDateBegin" name="createDateBegin"   />
						<input value="<fmt:formatDate value='${query.createDateEnd}' pattern='<%=SysUser.FORMAT_CREATE_DATE%>'/>" onclick="WdatePicker({dateFmt:'<%=SysUser.FORMAT_CREATE_DATE%>'})" id="createDateEnd" name="createDateEnd"   />
					</td>
				</tr>	
			</table>
		</fieldset>
